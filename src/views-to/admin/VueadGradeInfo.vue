  ...
  <p class="conTitle" id="selectLecArea">
    ...
    <span class="fr">
      ...
      >
        ...
        <option v-for="(seq, index) in detailcode" :value="seq" :key="index">
          {{ detailname[index] }}
        </option>
      </select>
      <a
    ...
    <table class="col">
      ...
      <template v-else>
        <tbody v-for="(list, index) in lecList" :key="index">
          <td>
            <a href="" @click.prevent="gradelistsearch(list.lec_seq)">{{
              list.lec_name
            }}</a>
          </td>
          <td>{{ list.lec_test }}</td>
          ...
          <td>
            <a
              ...
              href=""
              @click.prevent="enableschange(list.lec_seq)"
            >
  ...
  methods: {
    ...
    selectbox: function () {
      this.axios
        ...
        .then((result) => {
          this.detailcode = result.data.detailcode;
          this.detailname = result.data.detailname;
    ...
    leclistsearch: function (pagenum) {
      ...
      params.append('lectest', this.testtype);

      this.axios.post('/admin/vueadGradeList.do', params).then((result) => {
    ...
    enableschange: function (lecseq) {
      ...
      this.axios.post('/admin/adEnaChange.do', params).then(() => {
        this.leclistsearch();
      });
    },
    ...
    gradelist: function () {
      ...
      this.axios.post('/admin/vueadEnterList.do', params).then((result) => {
