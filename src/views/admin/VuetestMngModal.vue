<template>
  <dl style="background-color: white; width: 600px; height: 370px">
    <dt style="margin-bottom: 30px; background-color: #3e4463; height: 30px">
      <strong
        style="
          margin-left: 20px;
          font-size: 15px;
    ...
    <dd class="content" style="margin-left: 20px">
      ...
      <div class="tbl">
        <div class="seq">
          ...
          <div v-for="(val, idx) in checkedList" :key="idx">
            <input
              type="radio"
              name="radio"
              :id="idx"
...
export default {
  ...
  methods: {
    ...
    showmodal: function () {
      ...
      this.paction === 'U' && this.initqstlist();
    },
    // 시험등록 질문리스트
    ...
    testsave: function () {
      console.log(this.paction);
      if (this.paction === 'I' && !this.checkName()) {
        Swal.fire({
          title: '같은 시험명이 존재합니다.',
          icon: 'warning',
        });
      ...
      if (this.action !== 'D' && !this.checkMsg()) {
        return;
      }
      let vm = this;
      console.log(this);
      ...
      params.append('testtype', this.pdtestseq);
